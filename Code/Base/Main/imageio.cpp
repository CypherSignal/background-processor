...
Image loadImage(const std::filesystem::path& filename)
{
	...
	int ogHeight = 0;
	Image img;
	unsigned char *data = stbi_load(filename.generic_string().c_str(), &ogWidth, &ogHeight, &img.comp, 4);
	if (ogHeight % TileSize == 0 && ogWidth % TileSize == 0)
	{
		...
		img.comp = 4;
		memcpy(img.imgData.data(), data, img.width * img.height * 4);
	}
	else
	{
		...
		{
			memcpy(&img.imgData[row * img.width], &data[row * ogWidth * 4], ogWidth * 4);
		}
	}
